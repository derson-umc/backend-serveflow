...
class GetDashboardMetricsUseCaseTest {
    ...
    class Execute {
        ...
        @Test
        @DisplayName("calcula ticketMedio corretamente quando há clientes hoje")
        void execute_calculatesTicketMedio_whenCustomersPresent() {
        ...
        @Test
        @DisplayName("retorna ticketMedio ZERO quando não há clientes hoje")
        void execute_returnsZeroTicketMedio_whenNoCustomersToday() {
            when(repo.revenueToday()).thenReturn(new BigDecimal("0.00"));
            when(repo.ordersToday()).thenReturn(0L);
            when(repo.customersToday()).thenReturn(0L);
            when(repo.revenueYesterday()).thenReturn(new BigDecimal("100.00"));
            when(repo.ordersYesterday()).thenReturn(2L);
        ...
        @Test
        @DisplayName("retorna ticketMedioYesterday ZERO quando não há clientes ontem")
        void execute_returnsZeroTicketMedioYesterday_whenNoCustomersYesterday() {
            when(repo.revenueToday()).thenReturn(new BigDecimal("200.00"));
            when(repo.ordersToday()).thenReturn(4L);
